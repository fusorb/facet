...
program
  ...
  .addHelpText(
    "after",
    "\nDocs and examples: https://github.com/fusorb/facet/tree/main/packages/cli\n",
  );
...
docsCommand
  ...
  .option(
    "--use-template <name>",
    "Merge an existing template dir from ./templates into the scaffold",
  )
  .option(
    "--barrel <mode>",
    "Create a barrel export for the generated site: auto (create when it fits, default), always, or never",
  )
  .action(
    async (opts: {
      yes?: boolean;
      ...
      if (decided) {
        ...
        console.log(
          `  barrel:    ${answers.barrel === true ? "always" : answers.barrel === false ? "never" : "auto (create when it fits)"}`,
        );
        console.log(
          "Override any choice with the matching flag (e.g. --framework next).",
        );
      }
      ...
      if (opts.useTemplate) {
        ...
        if (!template) {
          ...
          if (merge.written.length) {
            for (const f of merge.written)
              console.log(`  ${path.join(target, f).replace(cwd, ".")}`);
          }
          if (merge.merged.length) {
            for (const f of merge.merged) {
              if (!merge.written.includes(f))
                console.log(
                  `  ${path.join(target, f).replace(cwd, ".")} (merged)`,
                );
            }
          }
          if (merge.conflicts.length) {
            ...
            for (const f of merge.conflicts)
              console.log(`  ${path.join(target, f).replace(cwd, ".")}`);
          }
        }
      }
      ...
      if (answers.framework === "react-vite") {
        console.log(
          `  ${installCommand(pm)} && pnpm dev   # open the docs site`,
        );
      } else if (answers.framework === "next") {
        console.log(
          `  ${installCommand(pm)} && pnpm dev   # docs live at /docs`,
        );
        ...
        console.log(
          "  Add markdown under ./content and run: python docs_pipeline.py > pages.json",
        );
      } else {
        console.log(
          "  Add markdown under ./content and run the content pipeline.",
        );
      }
      if (answers.useFacetTokens) {
        console.log(
          "  Theming: @fusorb/facet-tokens is wired via ThemeProvider + overrideVars.",
        );
      }
      console.log(
...
docsCommand
  ...
  .action(async (opts: { out?: string; yes?: boolean }) => {
    ...
    console.log(
      `  - Run \`facet docs init\` to mount the docs site, then point it at the drafted pages.`,
    );
  });
...
const iconsCommand = program
  .command("icons")
  .description("Icon registry commands");
iconsCommand
  .command("generate")
  .description(
  ...
  .action(async (opts: { path?: string; yes?: boolean }) => {
    ...
    console.log(
      `  Scanned ${scan.files.length} source file(s) for icon call sites.`,
    );
    if (scan.names.length) {
      console.log(
        `  Icons referenced: ${scan.names.length} (${scan.kebabNames.length} unique)`,
      );
    } else {
      console.log(
        "  No icon call sites found - writing the default semantic set.",
      );
    }
    if (resolved.renamed.length) {
      console.log(
        `  Legacy names mapped to current lucide icons: ${resolved.renamed.join(", ")}`,
      );
    }
    ...
    console.log(
      `  Writing ${resolved.used.size} icon(s) into ${scan.targetDir.replace(cwd, ".")}/`,
    );
    ...
    console.log("\nNext:");
    console.log(
      "  - Import GeneratedIcon from that file anywhere you use icons.",
    );
    console.log(
      "  - Re-run `facet icons generate` after adding/removing icons to keep the set exact.",
    );
  });

const emailsCommand = program
  .command("emails")
  .description("Email template commands");
emailsCommand
  .command("init")
  .description(
  ...
  .option(
    "--use-template <name>",
    "Merge an existing email template dir from ./templates into the scaffold",
  )
  .action(
    async (opts: {
      yes?: boolean;
      ...
      const cwd = process.cwd();
      const { detectMailSetup, planEmailsInit, formatDetection } =
        await import("./lib/emails.js");
      const { generateEmailsScaffold, emailsPackageJsonAdditions } =
        await import("./lib/emails-generators.js");
      const { writeFiles, readExistingPackageJson } =
        await import("./lib/writer.js");
      ...
      if (!opts.yes) {
        ...
        answers = {
          ...answers,
          mode: modeChoice.mode === "fresh" ? "fresh" : "migrate",
        };
      }

      const brandName = opts.name ?? path.basename(cwd);
      const latestEmails = await resolveLatestVersion("@fusorb/facet-emails");
      const facetEmailsRange = latestEmails ? `^${latestEmails}` : "latest";
      const files = generateEmailsScaffold(cwd, {
        ...answers,
        brandName,
        facetEmailsRange,
      });
      const { deps, scripts } = emailsPackageJsonAdditions({
        ...answers,
        brandName,
        facetEmailsRange,
      });
      ...
      const missing = Object.entries(deps).filter(
        ([name]) =>
          !detection.facetEmailsInstalled && name === "@fusorb/facet-emails",
      );
      if (missing.length) {
      ...
      const { buildRepoContext, generalRepoProvider, suggestRepoSteps } =
        await import("./lib/suggest.js");
...
const templatesCommand = program
  .command("templates")
  .description(
    "Template directory commands (consumer templates merged by docs init / emails init)",
  );
templatesCommand
  .command("list")
  .description(
    "List template directories found in this repo (./templates, ./docs/templates, ./emails/templates)",
  )
  .action(async () => {
    const cwd = process.cwd();
    ...
    if (!templates.length) {
      console.log(
        "No templates found. Create one under ./templates/<name>/ with an optional template.json manifest.",
      );
      return;
    }
...
templatesCommand
  ...
  .action(async (name: string) => {
    ...
    if (!template) {
      console.error(
        `Template "${name}" not found. Run \`facet templates list\` to see what's available.`,
      );
      process.exitCode = 2;
      return;
    }
    ...
    if (template.manifest?.description)
      console.log(`  description: ${template.manifest.description}`);
    if (template.manifest?.include?.length)
      console.log(`  include: ${template.manifest.include.join(", ")}`);
    if (template.manifest?.exclude?.length)
      console.log(`  exclude: ${template.manifest.exclude.join(", ")}`);
...
program
  ...
  .option(
    "--ui-dir <uiDir>",
    "Subdirectory that holds the copies (default: facet; ignored with --flat)",
    "facet",
  )
  .option(
    "--flat",
  ...
  .option(
    "--barrel",
    "Always create a barrel export (even when one does not exist yet)",
  )
  .action(
    async (
      component: string,
      ...
      if (facetPkg) {
        console.log(
          `\`facet copy\` copies components, but "${component}" resolves to ${facetPkg}.`,
        );
        console.log(
          `To install the package, run: facet add ${component}  (or facet install ${component})`,
        );
        process.exitCode = 1;
        return;
      }
      ...
      const answers = {
        language: (opts.js ? "javascript" : "typescript") as
      ...
      const barrel = written.find((f) =>
        f.endsWith(`/index.${answers.language === "javascript" ? "js" : "ts"}`),
      );
      if (barrel) console.log(`Barrel: ${barrel}`);
      console.log(
        "Note: importing from @fusorb/facet-components is recommended over copying source.",
      );
    },
  );
...
program
  ...
  .alias("add")
  .description(
    "Install one or more facet packages by shorthand (e.g. 'layout'), alias (e.g. 'facet-cli'), or full name (e.g. '@fusorb/facet-layout'). Alias: `facet add`. Pass multiple names to install them together in one go.",
  )
  .option(
    "-g, --global",
    "Install globally (e.g. `facet install -g facet-cli`) instead of locally",
  )
  .action(async (names: string[], opts: { global?: boolean }) => {
    ...
    if (unknown.length) {
      ...
      console.error(
        "Install by shorthand (e.g. 'layout', 'store', 'auth'), alias (e.g. 'facet-cli'), or full name (e.g. '@fusorb/facet-layout').",
      );
      for (const input of unknown) {
        console.error(
          `To copy the "${input}" component instead, use: facet copy ${input}`,
        );
      }
      process.exitCode = 1;
      return;
    ...
    const unique = Array.from(
      new Map(resolved.map((r) => [r.name, r])).values(),
    );
    ...
    vlog(
      `Package manager: ${pm} (workspace: ${workspace}, global: ${Boolean(opts.global)})`,
    );
    ...
    const withVersions = await Promise.all(
      unique.map(async ({ name }) => ({
        name,
        latest: await resolveLatestVersion(name),
      })),
    );
    ...
    if (unresolved.length) {
      for (const v of unresolved) {
        console.error(
          `Could not resolve ${v.name} on the npm registry. Is the package name correct?`,
        );
      }
      process.exitCode = 1;
      return;
    ...
    if (opts.global) {
      console.log(
        `Installing ${targets.map((t) => `${t.name}@${t.latest}`).join(", ")} globally (${pm})...`,
      );
    } else {
      console.log(
        `Installing ${targets.map((t) => `${t.name}@^${t.latest}`).join(", ")} (${pm})...`,
      );
    }
    const { execSync } = await import("node:child_process");
    try {
...
program
  ...
  .description(
    "Remove one or more installed facet packages by shorthand (e.g. 'layout'), alias (e.g. 'facet-cli'), or full name (e.g. '@fusorb/facet-layout'). Aliases: `facet rm`, `facet uninstall`.",
  )
  .option(
    "-g, --global",
    "Remove globally (e.g. `facet remove -g facet-cli`) instead of locally",
  )
  .action(async (names: string[], opts: { global?: boolean }) => {
    const cwd = process.cwd();
    ...
    if (unknown.length) {
      ...
      console.error(
        "Remove by shorthand (e.g. 'layout', 'store', 'auth'), alias (e.g. 'facet-cli'), or full name (e.g. '@fusorb/facet-layout').",
      );
      process.exitCode = 1;
      return;
    }
    ...
    console.log(
      `Removing ${unique.join(", ")}${opts.global ? " globally" : ""} (${pm})...`,
    );
    const { execSync } = await import("node:child_process");
    try {
      execSync(removePkg, { cwd, stdio: "inherit" });
      const label =
        unique.length === 1 ? unique[0]! : `${unique.length} facet packages`;
...
program
  .command("pkg")
  .description(
    "Show the latest published facet package versions and what this repo has installed",
  )
  .action(async () => {
    const cwd = process.cwd();
    ...
    if (outdated.length) {
      console.log(
        `${outdated.length} update(s) available. Run \`facet update\` to see how.`,
      );
    } else {
      console.log(
        "All installed facet packages are up to date (or not installed here).",
      );
    }
  });
...
program
  .command("doctor")
  .description(
    "Audit this repo: monorepo layout, facet deps, and best-practice suggestions",
  )
  .action(async () => {
    const cwd = process.cwd();
    const infos = await collectFacetPackageState(cwd);
    ...
    if (report.monorepo) {
      console.log("");
      console.log(
        "Monorepo detected: run \`facet update\` from the workspace root to update all members.",
      );
    }
  });
...
program
  ...
  .action(async (opts: { dryRun?: boolean; yes?: boolean }) => {
    ...
    for (const t of targets) {
      ...
      console.log(
        `  ${t.name}: ${info?.installed ?? "not installed"} -> ^${t.latest}`,
      );
    }
    console.log("");
    ...
    try {
      ...
      console.error(
        "facet update failed. Run the command manually to see the error:",
      );
      console.error(`  ${cmd}`);
      process.exitCode = 1;
    }
  ...
  .action(async (opts: { dryRun?: boolean }) => {
    ...
    if (!outdated.length) {
      if (unverified.length > 0) {
        ...
        console.log(
          "Run `facet update` once more, or check your network / registry access.",
        );
        process.exitCode = 2;
        return;
      }
    ...
    try {
      ...
      console.error(
        "facet up failed. Run the command manually to see the error:",
      );
      console.error(`  ${cmd}`);
      process.exitCode = 1;
    }
...
program
  ...
  .option(
    "--delete-local",
    "Also delete dead local component files (destructive; not run by default)",
  )
  .action(
    async (opts: {
      dryRun?: boolean;
      ...
      const workspace = detectMonorepo(cwd) !== null;
      vlog(
        `clean: pm=${pm} workspace=${workspace} manifests=${plan.manifests.length} imports=${plan.imports.length} deletable=${plan.deletableFiles.length}`,
      );

      if (
        plan.manifests.length === 0 &&
        plan.imports.length === 0 &&
        plan.deletableFiles.length === 0
      ) {
        console.log(
          "Nothing to clean: no bundled deps, no shadcn-style imports, no dead local components.",
        );
        return;
      }
      ...
      if (opts.dryRun) {
        ...
        for (const entry of plan.manifests) {
          console.log(
            `  ${entry.pkgName}: remove ${entry.deps.map((d) => d.name).join(", ")}`,
          );
        }
        for (const imp of plan.imports) {
          console.log(
            `  rewrite ${imp.from} -> @fusorb/facet-components (${imp.kind})`,
          );
        }
        if (plan.deletableFiles.length) {
          console.log(
            `  delete unused local components (requires --delete-local):`,
          );
          for (const f of plan.deletableFiles) console.log(`    ${f}`);
        }
        const names = [
          ...new Set(plan.manifests.flatMap((e) => e.deps.map((d) => d.name))),
        ];
        if (names.length)
          console.log(
      ...
      if (rewritten.length)
        console.log(`  Rewrote imports in ${rewritten.length} file(s)`);
      if (deleted)
        console.log(`  Deleted ${deleted} unused local component file(s)`);
      if (removed.length) {
...
program
  .command("scripts")
  .description(
    "Add useful npm scripts (docs, quality, facet:action, prep) to your package.json",
  )
  ...
  .action(async (opts: { yes?: boolean }) => {
    ...
    if (!selected.length) {
      const res = await prompts({
        ...
        name: "ids",
        message:
          "Which script presets should I add? (existing scripts are never overwritten)",
        choices: presetIds.map((id) => ({
          title: PRESET_SCRIPTS[id]!.label,
          value: id,
        })),
        instructions: false,
      });
    ...
    if (!result.content || result.added.length === 0) {
      console.log(
        "No new scripts to add (all requested scripts already exist).",
      );
      return;
    }
    const { writeFileSync } = await import("node:fs");
...
program
  .command("prep")
  .description(
    "Pre-go-live sync: check facet deps, doctor, and the consumer's build/typecheck/test",
  )
  .action(async () => {
    const cwd = process.cwd();
      ...
      try {
        if (label === "facet pkg") {
          ...
          if (outdated.length) {
            ...
            console.log(
              `    ${outdated.map((i) => i.name).join(", ")} - run \`facet up\`.`,
            );
            failed = true;
          } else {
            console.log("PASS");
          ...
          const infos = await collectFacetPackageState(cwd);
          const report = buildDoctorReport(cwd, infos);
          const problematic = report.findings.filter((f) =>
            f.includes("Unnecessary deps"),
          );
...
program
  ...
  .action(async (opts: { dryRun?: boolean }) => {
    ...
    if (!latest) {
      console.error(
        "Could not resolve the latest facet-cli version from the npm registry.",
      );
      process.exitCode = 1;
      return;
    }
    ...
    try {
      ...
      console.error(
        "Could not update globally (restricted environment without global permissions).",
      );
      console.log("");
      console.log("Trying ephemeral npx fallback...");
      try {
        execSync(`npx --yes @fusorb/facet-cli@latest --help`, {
          stdio: "inherit",
        });
        console.log("");
